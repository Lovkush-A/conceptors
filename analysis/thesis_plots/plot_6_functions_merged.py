...
def plot_data(ax, layers, additive, additive_std, additive_mean_centered, additive_mean_centered_std, baseline, baseline_std, conceptor, conceptor_std, conceptor_mean_centered, conceptor_mean_centered_std, title):
    conceptor_mc, = ax.plot(layers, conceptor_mean_centered, label='Conceptor Mean Centered', color='green', marker='o', linestyle='--', linewidth=1.5)
    conceptor_line, = ax.plot(layers, conceptor, label='Conceptor', color='mediumseagreen', marker='o', linewidth=1.5)
    additive_mc, = ax.plot(layers, additive_mean_centered, label='Additive Mean Centered', color='orange', marker='o', linestyle='--', linewidth=1.5)
    additive_line, = ax.plot(layers, additive, label='Additive', color='indianred', marker='o', linewidth=1.5)
    baseline_line, = ax.plot(layers, baseline, color='darkgray', linestyle='--', label='Baseline', linewidth=1.5)

    ax.errorbar(layers, conceptor_mean_centered, yerr=conceptor_mean_centered_std, fmt='none', ecolor='green', capsize=3)
    ax.errorbar(layers, conceptor, yerr=conceptor_std, fmt='none', ecolor='mediumseagreen', capsize=3)
    ax.errorbar(layers, additive_mean_centered, yerr=additive_mean_centered_std, fmt='none', ecolor='orange', capsize=3)
    ax.errorbar(layers, additive, yerr=additive_std, fmt='none', ecolor='indianred', capsize=3)

    ax.set_xlabel('Layer Modified', fontsize=10, color='black')
    ax.set_ylabel('Top-1 Accuracy (%) ± Std Dev', fontsize=10, color='black')
    ...
    # Remove the spines (the black border)
...
plt.savefig('../Plots/6_functions_merged.pdf', format='pdf', bbox_inches='tight')
plt.show()